...
class oxylus.utils.UMc {
	...
	/* Trace nested clips and build with vars
	 * 
	 * output example:
	 * play = buttons["play"];
	 * 
	 @author: Razvan Agache, razvan@oxylus.ro, www.oxylusflash.com 
	 @last_update: 08/01/2010 (dd/mm/yyyy) 
	 * 
	 * */
	public static function traceNestedClipsAndBuildVars(mc:MovieClip, mcName:String):Void {
		for (var p in mc) {
			if (String(p).indexOf("instance") == 0) continue;
			...
			if (type.length > 0) trace(p + " = " + mcName + "[" + String.fromCharCode(34) + p + String.fromCharCode(34) + "];")
		}
	}
}